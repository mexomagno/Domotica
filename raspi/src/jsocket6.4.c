...
/* Si se quiere descripción verbal de cada proceso */
 #define VERBOSE 0
...
int j_socket()
{
    ...
    if(fd < 0) { if (VERBOSE) fprintf(stderr, "no pude hacer un socket\n");
		  return -1;
	       }
    setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &sz, sizeof sz);
    return(fd);
}
...
/* mapear dirs IPv4 a IPv6!! */
...
#ifdef JDEBUG
	        inet_ntop(AF_INET6, portname.sin6_addr.s6_addr, name, sizeof name); 
	        if (VERBOSE) fprintf(stderr, "Trying %s/%d...", name, port);
#endif
#else /* JIPv6 */
		...
		else { if (VERBOSE) perror("connect"); if (VERBOSE) fprintf(stderr, "sock failed=%d\n", s);}
	    }
        } else if (VERBOSE) fprintf(stderr, "name fail!: %s\n", host);
	...
	if( ret != 0 ) /* Mmmh, puede ser que no soporte bien IPv6? */
	  {
	    if (VERBOSE) fprintf(stderr, "Name/port unknown: %s/%s err: %s\n", host, sport, gai_strerror(ret));
	    return(-1);
	  }
	...
	for(hp=addresses; hp != NULL; hp = hp->ai_next) {
	    ...
	    if (VERBOSE) fprintf(stderr, "Trying %s/%s...", name, sport);
#endif
 ...
 */
    ...
    while (res) {
        ...
        if (!(sockfd < 0)) {
            setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &sz, sizeof sz);
